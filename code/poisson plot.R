...
##### Fix data summary #####
load("Poissonfix0422.RData")

summarylist = list()
betatrue=rep(0, 1000); betatrue[1:10]=0.5*(-1)^{1:10}
MC = length(PoissonFix); M = dim( PoissonFix[[1]][[1]] )[2]
...
for( i in 1:7 ){ # i-th method
  ...
  for(j in 1:MC){
    methodi = methodi + apply( PoissonFix[[j]][[i]], 2, mysummary, real = betatrue )
    cat(j, "/", MC, "repetition, ", i, "/", 7, "method", "\r")
  }
  summarylist[[i]] = methodi/MC
}

FixPoisson = summarylist[-4]
...
plot_df <- lapply(1:length(FixPoisson), function(i) {
  mat <- FixPoisson[[i]]
...
PoissonFixPlot = ggplot(plot_df_final, aes(x = Batch, y = Value, color = Method, linetype = Method, shape = Method, group = Method)) +
...
ggsave("PoissonFixPlot22.pdf", PoissonFixPlot, width = 12, height = 10, dpi = 600)
...
load("Poissonincrease0423.RData")
summarylistInc = list()
betatrue = rep(0, 1000); betatrue[1:10]=0.5*(-1)^{1:10}
MC = length(PoissonIncrease); M = dim( PoissonIncrease[[1]][[1]] )[2]

library(stringr)

# dolog: make a logarithmic transformation (since error of RADAR is too high)
dolog=F
...
for( i in 1:7 ){ # i-th method
  ...
  for(j in 1:MC){
    methodi = methodi + apply( PoissonIncrease[[j]][[i]], 2, mysummary, real = betatrue )
    cat(j, "/", MC, "repetition, ", i, "/", 7, "method", "\r")
  }
  At = methodi/MC
  if(dolog == T){
...
plot_df <- lapply(1:length(IncPoisson), function(i) {
  mat <- IncPoisson[[i]]
  df <- as.data.frame(mat)
...
base_font_size <- 22
PoissonIncPlot = ggplot(plot_df_final, aes(x = Batch, y = Value, color = Method, linetype = Method, shape = Method, group = Method)) +
  geom_line(linewidth = 1.2) + 
  facet_wrap(~Metric, scales = "free", nrow = 2, axes = "all") +
...
ggsave("PoissonIncPlot24.pdf", PoissonIncPlot, width = 12, height = 10, dpi = 600)
